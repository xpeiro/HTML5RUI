...
joystick.addEventListener('mousedown', touch, false);
...
function mouseMove(evt) {
	...
	if (leftClick == 1) {//check if left mouse button down.
		...
		circle(joystickctx, point.x, point.y, radius, "grey");
		position.innerHTML = "x:" + (point.x - joystick.width / 2) + " y:" + (joystick.height / 2 - point.y);
...
function resetJoystick() {
	...
	circle(joystickctx, joystick.width / 2, joystick.height / 2, maxRadius, "#8e98a4");
...
function resetAll() {
	resetJoystick();
	resetVector();
	circle(joystickctx, joystick.width / 2, joystick.height / 2, radius, "grey");
}
